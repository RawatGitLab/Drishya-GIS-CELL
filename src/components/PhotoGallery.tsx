...
import { Search, Image, Calendar, MapPin, Trash2, SlidersHorizontal, ArrowUpRight, Download } from "lucide-react";
  ...
  const handleDownloadCSV = () => {
    ...
    const csvRows = [
      ...
      ...filteredPhotos.map((photo) => [
        escape(photo.id),
        escape(photo.metadata.title),
        escape(photo.metadata.description || ""),
        photo.metadata.lat,
        photo.metadata.lng,
        escape(`${window.location.origin}/api/photos/${photo.id}`),
        escape(photo.filename),
        photo.length,
        escape(photo.uploadDate)
      ].join(","))
    ];
    ...
    const filterText = searchQuery ? "_filtered" : "";
    link.setAttribute("download", `drishya_photos_metadata_${timestamp}${filterText}.csv`);
      ...
      <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
        ...
        {filteredPhotos.length > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 hover:text-emerald-800 rounded-xl transition-all border border-emerald-200/60 shadow-sm active:scale-95 cursor-pointer"
